...
def _station_action_available(state) -> bool:
    """Return True when a dock maintenance command can be started."""
    if state is None:
        return False
    if getattr(getattr(state, "working_status", None), "name", None) == "ERROR":
        return False
    return bool(getattr(state, "is_docked", False)) and not any(
        getattr(state, attr, False) is True
        for attr in (
            "is_cleaning",
            "has_recent_active_working_status",
            "is_returning",
            "is_charging_to_resume",
            "is_station_active",
        )
    )
...
class NarwalActionButton(NarwalEntity, ButtonEntity):
    ...
    def available(self) -> bool:
        ...
        return super().available and _station_action_available(self.coordinator.data)

    async def async_press(self) -> None:
        """Run the Narwal station action."""
        if not self.available:
            raise HomeAssistantError("Narwal station action cannot be started right now")

        client = self.coordinator.client
        if not client.robot_awake:
            await client.wake(timeout=10.0)

        if not _station_action_available(client.state):
            raise HomeAssistantError("Narwal station action cannot be started right now")
