...
class NarwalState:
    ...
    pending_active_working_status: tuple[Any, ...] | None = field(
    ...
    def update_from_working_status(self, decoded: dict[str, Any]) -> None:
        ...
        confirmed_external_clean = (
            ...
            and self.pending_active_working_status is not None
            and current_task_details != self.pending_active_working_status
        )
        ...
        if (
            ...
            if active_payload and not explicit_terminal_status:
                now = time.monotonic()
                if (
                    self.pending_active_working_status is None
                    or now - self.pending_active_working_status_time
                    > _TERMINAL_WORKING_STATUS_TTL
                ):
                    self.pending_active_working_status = current_task_details
