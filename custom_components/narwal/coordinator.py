...
CLOUD_CONSUMABLES_RESET_VERIFY_ATTEMPTS = 2
...
class NarwalCoordinator(DataUpdateCoordinator[NarwalState]):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._cloud_consumables_reset_lock = asyncio.Lock()
    ...
    def _reconcile_pending_new_clean_map_display(
        ...
        if (
            ...
            or self._has_clean_session_signal(state)
        ):
    ...
    async def async_reset_cloud_consumable(self, consumable_code: str) -> None:
        """Reset and verify one cloud consumable as a serialized transaction."""
        reset_lock = getattr(self, "_cloud_consumables_reset_lock", None)
        if reset_lock is None:
            reset_lock = asyncio.Lock()
            self._cloud_consumables_reset_lock = reset_lock
        async with reset_lock:
            ...
            if consumable.usage_duration <= 0:
                self.cloud_consumables_reset_error = None
                return
            try:
                await self._cloud_client.async_reset_consumable(
                    device_id=self.config_entry.data["device_id"],
                    product_id=configured_cloud_product_id(self.config_entry.data),
                ...
                for _attempt in range(CLOUD_CONSUMABLES_RESET_VERIFY_ATTEMPTS):
                    await asyncio.sleep(1.0)
                    await self.async_refresh_cloud_consumables(
                        force=True,
                        raise_on_error=True,
                    )
                    refreshed = self.cloud_consumables.get(consumable_code)
                    if (
                        refreshed is None
                        or refreshed.usage_duration < consumable.usage_duration
                    ):
                        self.cloud_consumables_reset_error = None
                        return
                raise NarwalCloudError(
                    f"Narwal did not confirm the {consumable.name} reset"
                )
            except NarwalCloudError as err:
