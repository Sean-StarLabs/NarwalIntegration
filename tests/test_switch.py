...
@pytest.mark.asyncio
async def test_dock_task_switch_revalidates_matching_task_after_wake() -> None:
    old_state = _state()
    old_state.update_from_working_status({"8": 9_000, "9": 18_000, "19": {}})
    new_state = _state()
    new_state.update_from_working_status({"12": 9_000, "13": 18_000, "19": {}})
    coordinator = _coordinator(old_state)
    coordinator.client.robot_awake = False

    async def wake(*, timeout: float) -> None:
        coordinator.client.state = new_state

    coordinator.client.wake = AsyncMock(side_effect=wake)
    coordinator.client.stop_dock_task = AsyncMock(
        return_value=CommandResponse(result_code=CommandResult.SUCCESS)
    )
...
def test_dock_task_switch_blocks_unverified_parallel_dry_start() -> None:
    state = _state()
...
def test_dock_task_switch_blocks_unverified_parallel_dry_start_reverse() -> None:
    state = _state()
    state.update_from_working_status({"10": 9_000, "11": 18_000, "19": {}})
    coordinator = _coordinator(state)
...
def test_dock_task_switch_blocks_unresolved_station_activity_starts() -> None:
    state = _state(station_activity=4)
    coordinator = _coordinator(state)

    assert state.is_station_active
    for description in DOCK_TASK_SWITCHES:
        switch = NarwalDockTaskSwitch(coordinator, description)
        assert switch.is_on is False
        assert not switch.available
