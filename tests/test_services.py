...
async def test_clean_rooms_service_wakes_unknown_before_map_resolution() -> None:
    """A sleeping UNKNOWN robot needs waking before get_map can resolve rooms."""
    coordinator = _coordinator()
    state = NarwalState(working_status=WorkingStatus.UNKNOWN)
    coordinator.client.state = state
    coordinator.data = state
    ...
    async def get_map() -> None:
        assert coordinator.client.robot_awake
        state.map_data = MapData(rooms=[RoomInfo(room_id=4), RoomInfo(room_id=7)])
