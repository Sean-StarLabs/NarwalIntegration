...
from narwal_client.models import CommandResponse  # noqa: E402
...
class TestNarwalSelect:
    ...
    async def test_select_option_rejects_when_entity_unavailable(self) -> None:
        coord = _coordinator(state=_state())
        ...
        assert coord.clean_settings.mop_strength == MopStrengthLevel.NORMAL
...
class TestLegacyNarwalSettingSelect:
    ...
    async def test_select_option_rejects_when_entity_unavailable(self) -> None:
        ...
        assert coord.clean_settings.route == CleaningRoute.METICULOUS
...
class TestNarwalPassesNumber:
    ...
    async def test_set_native_value_rejects_when_entity_unavailable(self) -> None:
        coord = _coordinator(state=_state())
        coord.last_update_success = False
        num = NarwalPassesNumber(coord)
        ...
        assert coord.clean_settings.passes == 1
