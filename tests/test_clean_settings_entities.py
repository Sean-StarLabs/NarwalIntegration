    ...
    *,
    ...
    def clear_room_clean_setting(
        ...
        *,
        ...
        key = (map_id if map_id is not None else room_settings_map_id(), room_id)
        customized = coord.room_clean_settings_customized.get(key)
        if not customized:
            return
        customized.discard(attr)
        if not customized:
            coord.room_clean_settings_customized.pop(key, None)
            coord.room_clean_settings.pop(key, None)
...
class TestRoomNarwalSettingSelect:
    ...
    async def test_inherit_option_clears_room_override(self) -> None:
        coord = _coordinator(state=_state())
        sel = RoomNarwalSettingSelect(coord, 4, "Kitchen", _ROOM_DESCS["route"])

        assert INHERIT_OPTION in sel.options
        coord.set_room_clean_setting(4, "route", CleaningRoute.STANDARD)
        coord.clean_settings.route = CleaningRoute.METICULOUS

        await sel.async_select_option(INHERIT_OPTION)

        assert coord.room_clean_settings_customized == {}
        assert (None, 4) not in coord.room_clean_settings
        assert sel.current_option == "Meticulous"

    async def test_inherit_option_can_clear_mode_specific_hidden_setting(self) -> None:
        coord = _coordinator(state=_state())
        coord.set_room_clean_setting(4, "work_mode", WorkMode.VACUUM)
        coord.set_room_clean_setting(4, "water", MopHumidity.WET)
        sel = RoomNarwalSettingSelect(coord, 4, "Kitchen", _ROOM_DESCS["water"])

        assert sel.available
        await sel.async_select_option(INHERIT_OPTION)

        assert coord.room_clean_settings_customized == {(None, 4): {"work_mode"}}
