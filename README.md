...
> - **Fan speed values and tiers changed** ([#49](https://github.com/sjmotew/NarwalIntegration/pull/49)). The suction scale was off by one tier for this project's entire history. The list is now the app's own tiers — Quiet, Standard, Strong, Super Powerful, Ultra Powerful. Existing `vacuum.set_fan_speed` and Narwal service calls using `quiet` / `normal` / `strong` / `max` or the earlier labels remain accepted, but they now map to the correct tier, so **actual suction may differ from what you were getting**. Select-entity automations must use the canonical labels shown by the entity.
