...
def _accumulated_trajectory_streams(
    decoded: dict[str, Any],
) -> tuple[bytes, bytes, tuple[int, int, int] | tuple[()]]:
    """Return raw trajectory streams and a deterministic change signature."""
    raw = decoded.get("2")
    ...
    if pair_bytes <= 0:
        return b"", b"", ()
    x_values = x_values[:pair_bytes]
    y_values = y_values[:pair_bytes]
    return (
        x_values,
        y_values,
        (
            ...
            zlib.crc32(y_values) & 0xFFFFFFFF,
        ),
    )
...
class MapDisplayData:
    ...
    def trajectory_points(self) -> list[tuple[float, float]]:
        ...
        return _decode_accumulated_trajectory(
    ...
    def from_broadcast(cls, decoded: dict[str, Any]) -> MapDisplayData:
        ...
        # Accumulated cleaning trajectory from Narwal itself. Keep raw streams
        # here so the event-loop callback can use the signature without decoding
        # the full route; the camera render executor decodes points.
        (
            result.trajectory_x_values,
            result.trajectory_y_values,
            result.trajectory_signature,
        ) = _accumulated_trajectory_streams(decoded)
